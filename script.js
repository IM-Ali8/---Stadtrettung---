...
// Boot
resetAll(true);
// ===== Mobile controls: extra-safe touch fallback (no refactor) =====
(function mobileTouchFallback(){
  const isTouch = true; // force enable on tablets too
  const mapHold = [
    ["btnLeft",  "ArrowLeft"],
    ["btnRight", "ArrowRight"],
    ["btnJump",  "Space"],
  ...
  mapHold.forEach(([id,code])=>holdBind(id,code));

  function tapBind(id, fn){
    const el = document.getElementById(id);
    if(!el) return;
    el.addEventListener("touchstart", (e)=>{ e.preventDefault(); e.stopPropagation(); fn(); }, {passive:false});
  }

  tapBind("btnStart", ()=>{ if(state===STATE.INTRO) startGame(); else resetAll(true); });
  tapBind("btnReset", ()=> resetAll(true));
})();